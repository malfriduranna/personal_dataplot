...
//         listeningData,
//         v => d3.sum(v, d => d.ms_played / 60000), // minutes
//         d => formatDay(d.ts) // group by day YYYY-MM-DD
//     );
//     const valueMap = new Map(dailyData);
...
//         // --- Calculate Monthly Totals (NEW) ---
//         const monthlyTotals = new Map(); // Map(Month Start Date Object -> total minutes)
//         valueMap.forEach((minutes, dayStr) => {
//             const dayDate = new Date(dayStr);
//             const monthStartDate = d3.timeMonth.floor(dayDate); // Get the start of the month
//             const monthKey = monthStartDate.toISOString(); // Use ISO string for reliable Map key
    
//             monthlyTotals.set(monthKey, (monthlyTotals.get(monthKey) || 0) + minutes);
//         });
    
//         // --- Find the Peak Month from Monthly Totals ---
//         let peakMonthKey = null; // Store the key (ISO string) of the peak month
//         let maxMinutesInPeakMonth = 0; // Store the total minutes of the peak month
//         monthlyTotals.forEach((totalMinutesInMonth, monthKey) => {
//             if (totalMinutesInMonth > maxMinutesInPeakMonth) {
//                 maxMinutesInPeakMonth = totalMinutesInMonth;
//                 peakMonthKey = monthKey;
//             }
//         });

//     let textContent = `<h3>Listening Summary: ${formatDate(initialStartDate)} to ${formatDate(initialEndDate)}</h3>`;
//     textContent += `<p>Total listening time: <strong>${formatTime(totalMinutes)}</strong> across ${numberOfDaysWithListening} days (out of ${totalDaysInPeriod} total days in the period).</p>`;
//     textContent += `<p>Average listening time per day (overall): ${formatTime(averageMinutesOverall)}.</p>`;
//     textContent += `<p>Average listening time per day (on days with listening): ${formatTime(averageMinutesPerListeningDay)}.</p>`;
//     if (peakDay) {
//         textContent += `<p>Peak listening day: <strong>${formatDate(new Date(peakDay))}</strong> with ${formatTime(maxMinutes)}.</p>`;
//     }
//     if (peakMonthKey) {
//         const peakMonthDate = new Date(peakMonthKey); // Convert the key back to a Date object
//         const formatFullMonthYear = d3.timeFormat("%B %Y"); // Format as "January 2023"
//         textContent += `<p>Peak listening month: <strong>${formatFullMonthYear(peakMonthDate)}</strong> with a total of ${formatTime(maxMinutesInPeakMonth)}.</p>`;
//     } else {
//         textContent += `<p>Peak listening month could not be determined.</p>`; // Fallback message
//     }

//     container.innerHTML = textContent;
//     // No handles or highlight needed for text version
//     // No legend needed
//     updateFilterInfoLabel(initialStartDate, initialEndDate); // Still update the label
// }
...
//      drawCalendarAsText(filteredData, viewStartDate, viewEndDate); // Added text version
