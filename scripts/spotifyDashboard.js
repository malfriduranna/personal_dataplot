...
(async function loadData() {
    ...
    if (allParsedData.length === 0) {
      ...
      const timelineChart = document.getElementById("timeline-chart");
      if (timelineChart)
        timelineChart.innerHTML = `<p class="empty-message">No data.</p>`;
    ...
    if (streamgraphContainer) {
      streamgraphContainer.innerHTML =
        '<p class="empty-message">Select a period in the calendar above to view Music vs Podcast rate.</p>';
      ...
      if (descEl) {
        ...
        console.log(
          "descEl.classList.contains:",
          typeof descEl.classList.contains
        );
    ...
    if (forceGraphContainer) {
      forceGraphContainer.innerHTML =
        '<p class="empty-message">Select a period in the calendar above to view artist transitions.</p>';
      const descEl = forceGraphContainer.nextElementSibling;
      if (descEl && descEl.classList.contains("chart-description")) {
        descEl.innerHTML =
          "Select a period in the calendar above to view artist transitions.";
      }
    }
  } catch (error) {
    console.error("Error loading or processing data:", error);
    // --- CORRECTED: Catch block ---
    if (calendarDiv)
      calendarDiv.innerHTML = `<p class="error-message">Error loading data. Check console for details.</p>`;
    if (filterInfoSpan) filterInfoSpan.textContent = "Error loading data";

    const timelineChart = document.getElementById("timeline-chart");
    if (timelineChart)
      timelineChart.innerHTML = `<p class="error-message">Error loading data.</p>`;
...
function drawCalendar(data, initialStartDate, initialEndDate) {
  ...
  if (listeningData.length === 0) {
    if (calendarDiv)
      calendarDiv.innerHTML = `<p class="empty-message">No listening data for this period.</p>`; // Added check
    const chartsToClear = [
      topArtistsUl,
      topTracksDiv,
      timeOfDayDiv,
  ...
  if (
    ...
    if (calendarDiv)
      calendarDiv.innerHTML = `<p class="error-message">Invalid date range.</p>`; // Added check
    if (typeof handleBrushUpdate === "function") handleBrushUpdate([]);
    return;
  }
  ...
  if (allDaysInCalendar.length === 0) {
    ...
    if (calendarDiv)
      calendarDiv.innerHTML = `<p class="error-message">Could not generate grid days.</p>`; // Added check
    if (typeof handleBrushUpdate === "function") handleBrushUpdate([]);
    return;
  }
...
function filterDataAndUpdateCharts(startDate, endDate) {
  if (
    ...
    console.warn(
      "filterDataAndUpdateCharts: Invalid date range or no data. Clearing dependent charts.",
      { startDate, endDate }
    );
    handleBrushUpdate([]);
  ...
  console.log(
    `Filtered data for ${formatDate(startDate)} to ${formatDate(endDate)}: ${
      filtered.length
    } records.`
  );
  updateFilterInfoLabel(startDate, endDate);
...
function drawLegend(container, scale, maxValue) {
  container.innerHTML = "";
  if (maxValue === undefined || maxValue <= 0) return;
...
function updateTopArtists(data) {
  ...
  artistData.forEach(([artist, totalMinutes], index) => {
    ...
    li.innerHTML = `<span class="artist-name">${
      index + 1
    }. ${artist}</span> <span class="artist-time">(${formatTime(
      totalMinutes
    )})</span>`;
...
function updateTopTracksChart(data) {
  const targetUl = document.getElementById("top-tracks-chart");
  if (!targetUl) return;
  targetUl.innerHTML = "";
  if (!data || data.length === 0) {
    targetUl.innerHTML = `<li class="empty-message">No data.</li>`;
  ...
  const trackData = d3
    .rollups(
      data.filter(
        (d) => d.track && d.track !== "Unknown Track" && d.ms_played > 0
      ),
      (v) => d3.sum(v, (d) => d.ms_played / 60000),
      (d) => d.track
    )
  ...
  if (trackData.length === 0) {
    targetUl.innerHTML = `<li class="empty-message">No Track data in this period.</li>`;
    return;
  }
  trackData.forEach(([track, totalMinutes], index) => {
    const li = document.createElement("li");
    li.innerHTML = `<span class="track-name">${
      index + 1
    }. ${track}</span> <span class="track-time">(${formatTime(
      totalMinutes
    )})</span>`;
    targetUl.appendChild(li);
  });
...
function updateTopTracksChart2(data) {
  ...
  const trackData = d3
    ...
    .slice(0, 5);
  if (trackData.length === 0) {
    /* ... no track data message ... */ return;
  }

  // --- Create List Structure ---
  const list = d3
    .select(targetDiv)
    .append("ol") // Use ordered list for ranking
...
function updateDayOfWeekChart(data) {
  ...
  const containerWidth = targetDiv.parentElement?.clientWidth || 200;
  const chartWidth = containerWidth > 0 ? containerWidth : 200;
  const chartHeight = 300;
  const width = chartWidth - chartMargin.left - chartMargin.right;
  const height = chartHeight - chartMargin.top - chartMargin.bottom;
  if (width <= 0 || height <= 0) {
...
function updateVisualization(filteredData) {
  ...
  if (!filteredData || filteredData.length === 0) {
    if (calendarDiv)
      calendarDiv.innerHTML = `<p class="empty-message">No data for selected period.</p>`; // Added check
  ...
  if (
    ...
    if (calendarDiv)
      calendarDiv.innerHTML = `<p class="error-message">Invalid date range in data.</p>`; // Added check
    if (legendDiv) legendDiv.innerHTML = ""; // Added check
    chartsToClear.forEach((el) => {
      if (el) el.innerHTML = `<p class="empty-message">No data.</p>`;
    });
...
async function drawForceGraph(filteredData, containerId, topN = 10) {
  ...
  if (aggregatedData.length === 0) {
    container.innerHTML =
      '<p class="empty-message">Could not aggregate data for proportions in this period.</p>';
    return;
  }
  ...
  const width = containerWidth - margin.left - margin.right;
  if (width <= 0 || height <= 0) {
    container.innerHTML = `<p class="error-message">Container too small for chart.</p>`;
    return;
  }
  const svg = d3
    .select(container)
  ...
  try {
    ...
    console.error("Streamgraph - Error during stacking:", error);
    container.innerHTML =
      '<p class="error-message">Error processing data for stacking.</p>';
    return;
  }
  if (series.length === 0 || !series[0] || series[0].length === 0) {
    const nonZeroTypes = contentTypes.filter((type) =>
      aggregatedData.some((d) => d[type] > 0)
    );
    if (nonZeroTypes.length === 1) {
      console.warn(
        `Streamgraph - Only found data for: ${nonZeroTypes[0]}. Drawing single layer.`
      );
      series = nonZeroTypes.map((key) => {
        const layer = aggregatedData.map((d, i) => {
          const point = [0, d[key]];
          point.data = d;
          return point;
        });
      ...
      container.innerHTML =
        '<p class="empty-message">No stack layers generated (no music/podcast data found?).</p>';
      return;
    }
  }
  const areaGen = d3
  ...
  svg
    ...
    .on("mouseover", (event, d_layer) => {
      ...
      const bisectDate = d3.bisector((d) => d.timeBin).left;
      const index = bisectDate(aggregatedData, hoveredDate, 1);
      const d0 = aggregatedData[index - 1];
      const d1 = aggregatedData[index];
      const closestData =
        d1 && d0 && hoveredDate - d0.timeBin > d1.timeBin - hoveredDate
          ? d1
          : d0;
      let tooltipContent = `<b>${d_layer.key}</b><br>(No time data)`;
      if (closestData) {
        tooltipContent = `<b>Time: ${d3.timeFormat(timeFormatString)(
          closestData.timeBin
        )}</b><br>Total Listen: ${formatTime(
          closestData.totalMinutes
        )}<br><hr>`;
        contentTypes.forEach((type) => {
          const percentage = (closestData[type] * 100).toFixed(1);
          const isHoveredType = type === d_layer.key;
          tooltipContent += `${
            isHoveredType ? "<b>" : ""
          }${type}: ${percentage}%${isHoveredType ? "</b>" : ""}<br>`;
        });
      }
      svg.selectAll(".stream-layer").style("fill-opacity", 0.3);
      d3.select(event.currentTarget)
        .style("fill-opacity", 1)
        .attr("stroke", "#000")
        .attr("stroke-width", 1.5);
      showTooltip(event, tooltipContent.trim());
    })
    .on("mousemove", moveTooltip)
    .on("mouseout", (event, d) => {
      svg
  ...
  if (
    ...
    descriptionElement.innerHTML =
      "Shows the proportional rate (%) of listening time between Music and Podcasts for the time period selected above.";
}
...
async function drawForceGraph(filteredData, containerId, topN = 10) {
  ...
  if (!container) {
    console.error(
      `drawForceGraph Error: Container element with ID "${containerId}" not found.`
    );
    return;
  }
  ...
  if (!filteredData || filteredData.length < 2) {
    container.innerHTML =
      '<p class="empty-message">Not enough data in this period to show transitions.</p>';
    return;
  }
  ...
  const nodeRadiusScale = d3
    .scaleSqrt()
    .domain([0, d3.max(nodes, (d) => d.playCount) || 1])
  ...
  const simulation = d3
    ...
    .force(
      ...
      d3
        .forceCollide()
        .radius((d) => nodeRadiusScale(d.playCount) + 5)
        .strength(0.7)
    ); // Increased collision buffer

  // --- Draw Links (Lines) - Appended to zoomableGroup ---
  const link = zoomableGroup
    .append("g") // Changed from svg to zoomableGroup
  ...
  const zoom = d3
    ...
    .extent([
      [0, 0],
      [width, height],
    ]) // Define panning area (within margins)
    .translateExtent([
      [0, 0],
      [width, height],
    ]) // Prevent panning outside the box
...
async function drawForceGraph2(filteredData, containerId, topN = 6) {
  const container = document.getElementById(containerId);
  ...
  if (!filteredData || filteredData.length < 2) {
    container.innerHTML =
      '<p class="empty-message">Not enough data in this period to show transitions (need at least 2 plays).</p>';
    const descEl = container.nextElementSibling;
    if (descEl && descEl.classList.contains("chart-description")) {
      descEl.innerHTML =
        "Select a period with more listening history to view artist transitions.";
    }
    return;
  }
  ...
  if (musicData.length < 2) {
    container.innerHTML =
      '<p class="empty-message">Not enough *music* plays in this period to show transitions.</p>';
    return;
  }
  ...
  if (transitions.size === 0) {
    container.innerHTML =
      '<p class="empty-message">No transitions found *between* the top artists in this period.</p>';
    return;
  }
  ...
  if (width <= 0 || height <= 0) {
    container.innerHTML =
      '<p class="error-message">Container is too small to draw the graph.</p>';
    return;
  }
  ...
  zoomableGroup
    .append("defs")
    .append("marker") // Append defs to zoomableGroup
    .attr("id", "arrowhead")
  ...
  // --- Scales ---
  const minRadius = 5,
    maxRadius = 15; // Slightly larger radii
  ...
  const simulation = d3
    ...
    .force(
      ...
      d3
        ...
        .strength(
          (link) => 1 / Math.min(link.source.playCount, link.target.playCount)
        ) // Weaker links between popular nodes
    )
  ...
  link
    .append("title") // Basic HTML tooltip for links
    .text((d) => `${d.source.id} → ${d.target.id}\n${d.value} transitions`);
  ...
  node
    .append("title") // Basic HTML tooltip for nodes
    .text((d) => `${d.id}\n${d.playCount} plays in period`);
  ...
  link
    .on("mouseover", highlightLink) // Optional: highlight link itself slightly more
  ...
  function highlight(event, d_hovered) {
    ...
    link
      ...
      .style("fill", (l) =>
        l.source === d_hovered || l.target === d_hovered ? "#555" : "#ccc"
      ); // Darken arrow if connected
  ...
  function unhighlightLink(event, d_hovered) {
    ...
    const relatedNode = d3
      .select(".force-nodes circle[style*='stroke: black']")
      .datum(); // Find if a node is actively hovered
    if (relatedNode) {
