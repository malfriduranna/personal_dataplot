...
d3.csv("data/astrid_data.csv")
  .then((rawData) => {
    ...
    if (rawData.columns.includes("master_metadata_track_name")) {
      ...
      console.warn(
        "'master_metadata_track_name' column not found. Top Tracks chart will be disabled."
      );
    }
    ...
    allParsedData = rawData
      ...
      .filter(
        (d) =>
          ...
          !isNaN(d.ms_played) &&
          d.ms_played >= 0
      ); // Ensure ms_played is not negative
...
function drawCalendar(data, onBrushEndCallback) {
  ...
  if (listeningData.length === 0) {
    ...
    if (typeof handleBrushUpdate === "function") {
      // Check if handler exists before calling
      handleBrushUpdate([]);
    } else {
      // Fallback if called outside the main flow
      [topArtistsUl, topTracksDiv, timeOfDayDiv, dayOfWeekDiv].forEach(
        (el) => (el.innerHTML = `<p class="empty-message">No data.</p>`)
      );
    }
    return;
  }
  ...
  if (datesInCurrentView.length === 0) {
    // Should be prevented by listeningData check, but safe
  ...
  if (
    !dataStartDate ||
    !dataEndDate ||
    !(dataStartDate instanceof Date) ||
    !(dataEndDate instanceof Date) ||
    isNaN(dataStartDate) ||
    isNaN(dataEndDate)
  ) {
    console.error(
      "drawCalendar: Invalid date range derived from data.",
      dataStartDate,
      dataEndDate
    );
  ...
  const firstDayOfNextMonthEnd = d3.timeMonth.offset(
  ...
  if (allDays.length === 0) {
    console.error(
      "drawCalendar: No days generated for the grid range.",
      firstDayOfMonthStart,
      firstDayOfNextMonthEnd
    );
  ...
  const cells = svg
    ...
    .on("mouseout", (event) => {
      ...
      d3.select(event.currentTarget)
        .attr("stroke", "#ffffff")
        .attr("stroke-width", cellPadding); // Restore border
    });
  ...
  const brush = d3
    ...
    .extent([
      [0, 0],
      [width - leftPadding - 20, 7 * (cellSize + cellPadding)],
    ])
  ...
  function brushed(event) {
    ...
    if (!event.selection) {
      ...
      console.log(
        "Brush cleared. Updating charts with full data for current view."
      );
    ...
    const selectedDates = allDays.filter((d) => {
      const cellX =
        d3.timeWeek.count(startDay, d) * (cellSize + cellPadding) +
        cellSize / 2;
    ...
    if (selectedDates.length > 0) {
      ...
      svg
        ...
        .text(
          `Selected: ${formatDate(brushStartDate)} → ${formatDate(
            brushEndDate
          )}`
        );
      ...
      const filteredForBrush = data.filter((d) => {
        ...
        return (
          ...
          dDate >= brushStartDate &&
          dDate < dayAfterBrushEnd
        );
      });
      console.log(
        `Brush active. Updating charts with ${filteredForBrush.length} brushed records.`
      );
      callback(filteredForBrush); // Update dependent charts based on brushed selection
    } else {
      console.log(
        "Brush active but selected no valid dates. Clearing dependent charts."
      );
...
function updateTopTracksChart(data) {
  ...
  const chartHeight =
    trackData.length * 25 + chartMargin.top + chartMargin.bottom; // Dynamic height
  ...
  svg
    ...
    .each(function (d) {
      ...
      while (
        self.node().getComputedTextLength() > maxWidth &&
        text.length > 10
      ) {
        text = text.slice(0, -4) + "...";
        self.text(text);
...
function handleBrushUpdate(brushedData) {
  console.log(
    "handleBrushUpdate called with data count:",
...
function updateVisualization(filteredData) {
  console.log(
    "updateVisualization: Updating with data count:",
...
yearSelect.onchange = () => {
  ...
  const filteredByYear = allParsedData.filter(
    (d) => d.ts.getFullYear() === selectedYear
  );
